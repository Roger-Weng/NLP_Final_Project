...
from glue_evaluator_ablation1_sean_test import GLUEvaluator, set_seed
...
RAND_BIAS_MASK_SIZE = {'bert-base-cased': 43324}
...
def _parse_args():
    ...
    parser.add_argument('--fine-tune-type', '-f', required=True, type=str,
                        ...
                        choices={'full_ft', 'bitfit', 'frozen', 'rand_bias', 'rand_uniform', 'rand_row_col'})
...
def _perform_training_preparations(evaluator, args, trainable_components):
    ...
    elif args.fine_tune_type in {'bitfit', 'frozen', 'rand_bias'}:
        evaluator.training_preparation(learning_rate=args.learning_rate,
                                       optimizer=args.optimizer,
                                       encoder_trainable=False,
                                       trainable_components=trainable_components,
                                       verbose=args.verbose)
        if args.fine_tune_type=='rand_bias':
            evaluator.set_uniform_bias_mask(mask_size=RAND_BIAS_MASK_SIZE[args.model_name])
    else:
        evaluator.training_preparation(learning_rate=args.learning_rate,
                                       optimizer=args.optimizer,
                                       encoder_trainable=True,
...
def main():
    ...
    # saving artifacts
    if args.fine_tune_type == 'bitfit' or 'rand_bias':
